...
export async function logAbsence(ctx, body) {
  ...
  // Previous values are read before the write so the audit trail can show what
  // an overwrite replaced.
  const { rows: beforeRows } = await sql.query(
    'select date, type, minutes_late, note from tp_attendance where person_id = $1 and date = any($2::date[])',
    [person.id, working]
  )
  const previous = new Map(beforeRows.map(r => [String(r.date).slice(0, 10), r]))

  // One statement, not a loop. This driver is autocommit per statement over
  // HTTP, so a loop of inserts has no transaction around it: a failure on day
  // three of five used to leave two days written and return an error, which is
  // a half-recorded absence nobody knows to clean up.
  await sql.query(
    `insert into tp_attendance (person_id, date, type, note, created_by)
     select $1, d, $2, $3, $4 from unnest($5::date[]) as d
     on conflict (person_id, date) do update
       set type = excluded.type, note = excluded.note,
           minutes_late = 0, login_time = null,
           created_by = excluded.created_by, updated_at = now()`,
    [person.id, type, note, ctx.user.id, working]
  )
  ...
  for (const date of written) {
    await audit({
      actor: ctx.user,
      action: previous.has(date) ? 'attendance.overwrite' : 'attendance.create',
      entity: 'attendance', personId: person.id,
      before: previous.get(date) || null, after: { date, type, note },
    })
  }
