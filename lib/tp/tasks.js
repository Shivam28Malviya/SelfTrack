...
export async function updateTask(ctx, taskId, body) {
  ...
  // Required for the same reason as on a person: an optional concurrency check
  // protects only the callers who opt in, which is nobody in a hurry.
  if (!expected) throw new HttpError(400, 'updatedAt is required so a concurrent edit can be detected.')
  if (new Date(expected).getTime() !== new Date(current.updated_at).getTime()) {
