...
// TeamPulse reuses SelfTrack's users.role column rather than inventing a second
// identity system. See docs/teampulse-spec.md section 2.
const ROLE_MAP = {
  admin: 'admin',
  moderator: 'manager',
  user: 'member',
...
export async function requireTp(req) {
  ...
  if (role === 'manager') {
    if (!self) throw new HttpError(403, 'No TeamPulse profile is linked to this account.')
  ...
  // Spectators see names and team-level aggregates, never per-person detail.
  return { role, user, self, scope: 'none', ids: [] }
}
