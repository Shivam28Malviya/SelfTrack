...
export async function heatmap(ctx, { personId = null } = {}) {
  // personId limits the grid to one row. The profile only ever draws its own
  // person, and building the whole team's grid to find it made one profile
  // load every rating in the team.
...
export async function listCerts(ctx, query = {}) {
  ...
  const certs = rows.map(r => ({
    ...
    issuer: r.issuer,
    kind: r.kind,
    // Derived, not stored. A lapsed certificate is expired whether or not
    // anyone updated the row, and this used to be an UPDATE on every GET —
    // a write on a read path, running even when the caller only wanted a count.
    status: r.status === 'completed' && r.expires_on && String(r.expires_on).slice(0, 10) < today
      ? 'expired'
      : r.status,
...
/** Just the number expiring inside the horizon — the overview wants a count,
 *  not the whole list. */
export async function expiringCertCount(ctx, { days = 90 } = {}) {
  const ids = ctx.scope === 'all' ? null : ctx.ids
  if (ctx.scope === 'none' || (ids && ids.length === 0)) return 0
  const { rows } = await sql.query(
    `select count(*)::int as n from tp_cert c join tp_person p on p.id = c.person_id
      where p.active = true and c.expires_on is not null
        and c.expires_on >= $2 and c.expires_on <= $3
        and ($1::bigint[] is null or c.person_id = any($1::bigint[]))`,
    [ids, todayISO(), addDays(todayISO(), days)]
  )
  return rows[0].n
}
