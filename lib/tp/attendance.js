...
export async function decideLeave(ctx, requestId, body) {
  ...
  if (decision === 'approved') {
    ...
    // One statement: this driver is autocommit per statement, so a loop would
    // leave an approval half-written if it failed partway through — and the
    // request would already be marked approved.
    await sql.query(
      `insert into tp_attendance (person_id, date, type, note, created_by)
       ...
       on conflict (person_id, date) do update
         set type = excluded.type, note = excluded.note,
             minutes_late = 0, login_time = null, updated_at = now()`,
      [request.person_id, request.type, `Approved leave #${id}`, ctx.user.id, days]
    )
    written = days.length
