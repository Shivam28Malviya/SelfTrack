...
export async function createSession(userId) {
  ...
  // opportunistic purge of expired sessions so abandoned tokens don't pile up
  await sql`delete from sessions where created_at < now() - interval '30 minutes'`
...
// Resolves a raw session token to a user row, or null. Expired sessions
// (older than the TTL) are deleted on touch.
