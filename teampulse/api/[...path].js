// The Team Pulse API. One catch-all function, mounted at /api/*, holding
// authentication and every Team Pulse route. Row-level access is decided in
// lib/tp/roles.js and nowhere else.
import {
  HttpError, requireAuth, verifyPassword, createSession, destroySession,
} from '../lib/auth.js'
...
import { pushNotif } from '../lib/notify.js'
import {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (route === '/auth/login' && method === 'POST') {
      ...
      const { rows } = await sql`select * from users where lower(email) = lower(${String(email || '').trim()})`
      const user = rows[0]
      // One message for both cases: saying which of the two was wrong tells an
      // attacker which addresses exist.
      if (!user || !(await verifyPassword(password || '', user.password_hash))) {
        throw new HttpError(401, 'That email and password do not match.')
      }
      if (user.status !== 'approved') throw new HttpError(403, 'This account is not approved yet.')
    ...
    if (leaveDecideMatch && method === 'POST') {
      ...
      // Tell the person. A decision nobody is told about is not a decision
      // they can act on.
