// Runs the whole app on localhost: the API and the built client.
//
// `vite dev` serves the client alone, with no API behind it. This mounts the
// same handler that runs in production under node:http, so what runs here is
