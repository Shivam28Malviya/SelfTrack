...
export default function TpTask() {
  ...
  const t = data.task
  const facts = [
    ['Owner', t.ownerId ? <Link to={`/people/${t.ownerId}`}>{t.ownerName}</Link> : 'Unassigned'],
    ...
    <TpLayout title={t.title}>
      <section className="tp-panel flex flex-col gap-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <Link to="/delivery" className="text-sm hover:opacity-70">← Delivery</Link>
          {(role === 'admin' || role === 'manager') && (
            <Link to={`/tasks/${id}/edit`} className="tp-btn-ghost">Edit</Link>
          )}
        </div>
