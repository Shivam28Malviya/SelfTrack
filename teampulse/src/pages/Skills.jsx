    ...
    <TpLayout title="Skills">
            ...
            <ul className="flex flex-wrap gap-2.5 m-0 p-0 list-none text-sm">
              {matches.map(m => (
                <li key={`${m.personId}-${m.skill}`}>
                  <Link to={`/people/${m.personId}`} className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white">
                    <span>{m.name}</span>
      ...
      {data.skills.length === 0 ? (
        <TpEmpty
          ...
          action={role === 'admin' && <Link to="/settings" className="tp-btn">Open settings</Link>}
        />
      ) : (
