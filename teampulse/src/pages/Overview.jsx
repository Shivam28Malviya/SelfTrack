      ...
      <div className="grid gap-5 lg:grid-cols-[2fr_1fr] items-start">
        <div className="tp-card flex flex-col gap-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            <Link to="/people" className="tp-btn-ghost">All people</Link>
          </div>
          ...
          {flagged.length === 0 ? (
            <TpEmpty
              ...
              action={headcount === 0 && (role === 'admin' || role === 'manager')
                ? <Link to="/people/new" className="tp-btn">Add person</Link>
                : undefined}
            />
          ) : (
                  ...
                  {flagged.map(p => (
                    <tr key={p.personId}>
                      <td className="border-t px-3 py-3 text-sm" style={{ borderColor: 'var(--tp-line)' }}>
                        <Link to={`/people/${p.personId}`} className="flex items-center gap-3">
        ...
        <div className="flex flex-col gap-5">
          ...
          {data.expiringCerts > 0 && (
            <Link to="/skills" className="rounded-[20px] px-5 py-4 text-sm"
