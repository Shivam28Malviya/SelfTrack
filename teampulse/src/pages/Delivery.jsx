    ...
    <TpLayout title="Delivery">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            {role !== 'spectator' && (role === 'admin' || role === 'manager') && (
              <Link to="/tasks/new" className="tp-btn">New task</Link>
            )}
      ...
      {!hasAnyTask ? (
        <TpEmpty
          ...
          action={(role === 'admin' || role === 'manager') && <Link to="/tasks/new" className="tp-btn">New task</Link>}
        />
      ) : (
        <>
                ...
                {(tasks?.tasks || []).map(task => (
                  <li key={task.id} className="rounded-3xl p-4 flex flex-col gap-2"
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <Link to={`/tasks/${task.id}`} className="text-sm">{task.title}</Link>
                  ...
                  <tbody className="tp-rows tp-stagger">
                    {(tasks?.tasks || []).map(task => (
                      <tr key={task.id}>
                        ...
                        <td className="border-t px-3 py-3 text-sm" style={{ borderColor: 'var(--tp-line)' }}>
                          {task.ownerId
                            ? <Link to={`/people/${task.ownerId}`}>{task.ownerName}</Link>
