  ...
  return (
    <TpLayout title={p.name}>
      <div className="flex flex-wrap items-center justify-between gap-3 px-2">
        <Link to="/people" className="text-sm hover:opacity-70">← All people</Link>
        <div className="flex flex-wrap gap-3">
          {canEdit && <Link to={`/entry?personId=${p.id}`} className="tp-btn">Log for {p.name.split(' ')[0]}</Link>}
          {canEdit && <Link to={`/people/${id}/edit`} className="tp-btn-ghost">Edit</Link>}
          {canEdit && p.active && (
