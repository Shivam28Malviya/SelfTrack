...
// This is the whole API, so calls go to /api/* directly. apiCall adds the
// /api prefix, the bearer token and the 401 redirect.
const qs = (params) => {
...
export const tpGet = (path, params) => apiCall('GET', `${path}${qs(params)}`)
export const tpPost = (path, body) => apiCall('POST', path, body)
export const tpPut = (path, body) => apiCall('PUT', path, body)
export const tpDelete = (path) => apiCall('DELETE', path)
