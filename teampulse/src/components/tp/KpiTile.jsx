 ...
 */
/** A score of 4.3 must keep its decimal; a count of 9 must not gain one. */
const decimalsFor = (v) => (Number.isInteger(Number(v)) ? 0 : 1)

export default function KpiTile({ label, value, unit = '', target, basis, tone = 'plain', hint, index = 0 }) {
  ...
  return (
    <div
      className="tp-rise rounded-[28px] px-5 py-4 flex flex-col gap-1.5"
      style={{ background: bg, color: fg, animationDelay: `${stagger(index)}ms` }}
    >
      <span className="text-3xl tracking-[-0.03em] tabular-nums">
        {has
          ? <CountUp value={value} decimals={decimalsFor(value)} suffix={unit} />
          : DASH}
