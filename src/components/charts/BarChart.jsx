      ...
      {data.map((d, i) => {
        ...
        return (
          <div key={i} className="flex-1 flex flex-col items-center justify-end h-full group">
            <span className="text-[10px] text-slate-300 mb-1 opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            />
            <span className="text-[10px] text-slate-300 mt-1 truncate max-w-full">{d.label}</span>
          </div>
        )
      })}
    </div>
